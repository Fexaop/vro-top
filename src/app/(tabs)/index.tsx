...
export default function DashboardScreen() {
  ...
  const overallAtt = attendance?.length
    ...
    : null;
  const gradedCourses = grades?.filter((g) => g.gradePoint != null && g.gradePoint > 0) ?? [];
  const gradedCredits = gradedCourses.reduce((s, g) => s + g.credits, 0);
  const cgpa = gradedCredits > 0
    ? (gradedCourses.reduce((s, g) => s + g.gradePoint! * g.credits, 0) / gradedCredits).toFixed(2)
    : null;
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text variant="headlineMedium" style={{ marginBottom: 16 }}>Dashboard</Text>

        <View className="flex-row gap-3 mb-4">
          <Card mode="elevated" style={[{ flex: 1 }, { backgroundColor: colors.primaryContainer }]} onPress={() => router.push('/(tabs)/attendance')}>
          ...
          <Card mode="elevated" style={[{ flex: 1 }, { backgroundColor: colors.secondaryContainer }]} onPress={() => router.push('/(tabs)/grades')}>
            <Card.Content style={{ alignItems: 'center', paddingVertical: 16 }}>
        ...
        {lowAtt.length > 0 ? (
          <Card mode="outlined" style={[{ marginBottom: 12 }, { borderColor: colors.error }]} onPress={() => router.push('/(tabs)/attendance')}>
            <Card.Content>
        ...
        {nextExam != null ? (
          <Card mode="outlined" style={{ marginBottom: 12 }} onPress={() => router.push('/(tabs)/exam')}>
            <Card.Content>
        ...
        <View className="flex-row flex-wrap gap-3">
          {[
            ...
            <Card key={label} mode="outlined" style={{ width: '47%' }} onPress={() => router.push(route as never)}>
              <Card.Content style={{ alignItems: 'center', paddingVertical: 16 }}>
