...
// Step 1: prelogin — ONE attempt per invocation (CF Workers subrequest limit).
// Returns { recaptcha: true } when VTOP shows Google reCAPTCHA — client retries with a fresh invocation.
export async function handleVtopPrelogin(c: Context): Promise<Response> {
  try {
    ...
    if (!csrf) return c.json({ error: 'Cannot extract CSRF — VTOP may be down' }, 503);
    ...
    // Tell client to retry — VTOP is showing reCAPTCHA this round
    if (loginHtml.includes('gResponse') || loginHtml.includes('g-recaptcha')) {
      return c.json({ recaptcha: true }, 200);
    }
